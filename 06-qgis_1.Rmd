...
QGISil on alati kaks hetkeversiooni: stabiilne *Long term release* ja uusimate võimalustega *Latest release*. Väga suuri erinevusi nende versioonide vahel ei ole. Kursusel kasutame uusimat versiooni, et tagada parim kasutajamugavus ka juhuks, kui tahate QGISi kasutada pärast kursuse lõppu. Siis olete järjest lisanduvate funktsionaalsustega juba mõneti tuttavad. Kel huvi, võib vaadata ka [väljalaskegraafikut](https://qgis.org/en/site/getinvolved/development/roadmap.html#release).

QGIS 3.20 saab alla laadida [ametlikult kodulehelt](https://qgis.org/en/site/forusers/download.html), valides enda arvuti operatsioonisüsteemile sobiva variandi. Windowsi jaoks *QGIS Standalone Installer Version 3.20* ja macOSi jaoks *QGIS macOS Installer Version 3.20*. Kui allalaadimine on lõppenud, ava fail ning installi see nagu iga teine programm. Kasuta vaikimisi valikuid.   

```{r qgis_3.1, echo=F, fig.show='hold', fig.align='center', out.width= "50%"}
knitr::include_graphics(here::here("imgs", "5_qgis_3.20.png"))
```

## Andmed ja failihaldus

Segaduste vältimiseks soovitame kasutada kaustade struktuuri, kus selle kursuse kaustas on teil QGISi projekti fail(id) .qgz ja kaks kausta **andmed** ja **kaardid**, kuhu panete vastavalt kõik praktikumites kasutatavad andmed ja valminud kaardid. Võimalike probleemide vältimiseks võiks kursuse kaust olla kohas (*directory path*), kus ei ole täpitähti. Nt *C:/ut/magistrantuur/sygis_2021/gis_humanitaarias/*.
...
- *CRS for Projects* all vali *Use a default CRS*, kliki rippmenüü kõrval ikoonil ja kirjuta seal otsinguse *Estonian Coordinate System 1997* ehk [L-EST97 koordinaatsüsteem](https://geoportaal.maaamet.ee/est/Ruumiandmed/Geodeetilised-andmed/Eesti-geodeetiline-susteem/L-EST97-koordinaatsusteem-p173.html). Vajuta sellele otsingu tulemusele ning menüü alumises servas *OK*. Tee sama *CRS for Layers* juures. 
...
```{r defaultcrs, echo=F, fig.show='hold', out.width= "100%", fig.cap="Vaikimisi koordinaatsüsteemi sätestamine", fig.align='center'}
knitr::include_graphics(here::here("imgs", "default_crs.gif"))
```

Vaata QGISi kasutajaliidest. Suurde keskel olevasse aknasse ilmuvad kaardid ja ruumiandmete kihid. Vasakul on kihtide aken *Layers*. All servas näitab kursori asukoha koordinaate, mõõtkava, mida saab seal ka muuta, ning paremal nurgas on koordinaatsüsteem, mis peaks praegu olema EPSG:3301, mis tähendabki Eesti 1997. a koordinaatsüsteemi. Üleval servas on menüüd ja kõiksugused tööriistad.  

## Andmete avamine

Nagu varasematest mäletate, on olemas raster- ja vektorandmed. Viimased neist võivad koosneda punktidest, joontest või aladest. Avame kõigist andmetüüpidest näite.

- Ava Eesti maakondade kiht.  
    - *Layer → Add layer → Add vector layer*.  
    - Kliki *Vector Dataset(s)* juures sirvimise ikoonil (kolm punkti). Vali andmete kaustast *shape*-fail *maakond_20201001.shp* ja vajuta *Open*. Seejärel menüü all servas *Add* ja *Close*.  
    - Nüüd on avatud kiht kõigi Eesti maakondade piirjoontega.  

- Ava muinsuskaitse all olevate kirikute kiht. Kõik samamoodi nagu eelmise kihiga, valida tuleb aga *geopackage*'i fail *kirikud.gpkg*.

- Ava suuremate jõgede kiht *joed.gpkg*. Kuna nüüd on selge, kuidas käib menüü kaudu kihi avamine, siis võite selle kihi avamiseks faili lihtsalt kaardiaknasse lohistada. 


Kihtide paneelil *Layers* saab kihte välja ja sisse lülitada ning hiirega tõstes muuta nende järjekorda. Peaaknas saab hiire rullikuga suumida (all servas muutub ka mõõtkava). Hiire vasakpoolset klahvi all hoides ja ka klaviatuuri nooltega saab liikuda. Kihi terve ulatuse nägemiseks tee *Layers* paneelil kihil parempoolne klikk ning vali *Zoom to Layer*. Sellest on abi ka siis, kui oled kaardil nö eksinud ja ei suuda kihti enam üles leida. Kihtide atribuuttabelit saab vaadata tehes paremkliki ja *Open Attribute Table*. Proovi kaardil ringi liikuda ning vaata kõikide kihtide atribuuttabelit!
...
Sümbolite muutmiseks tee kihil topeltkliki ja vaata, et avanenud aknas oleks vasakul servas valitud *Symbology*. Avanenud menüüs saab punkt- ja joonobjektidele valida värvi ja suurus või laiust. Pindobjektidel ehk aladel valige *Fill* all *Simple fill* asemel *Outline: Simple line*. See tähendab, et kogu ala pole mingi värviga täidetud, vaid näeme ainult ala piirjooni.

Alternatiivina võib kihi stiili muutmiseks sisse lülitada paneeli *Layer Styling*. Selleks üleval menüüriba *View → Panels → Layer Styling*. Nüüd on seesama *Symbology* aken kaardiakna kõrval ning muudatused on kohe paremini näha. 

Lisa maakonnanimi valides samas menüüs vasakult *Labels* ning seal *No labels* asemel *Single labels*. *Value* alt vali *MNIMI* ja vajuta all paremal OK.

```{r harjutus1, echo=F, fig.show='hold', out.width= "100%", fig.cap="Harjus 1 tulemus", fig.align='center'}
knitr::include_graphics(here::here("imgs", "harjutus_1.png"))
```

Kas see on hea visualiseerimine? Mis on hea, mis on puudu, mis on üleliigne? Võib mõelda, et eesmärgiks on näidata kaitse all olevate kirikute arvu maakonnas.


### Harjutus 2 

Harjutuse eesmärk on luua kaart, mis näitab maakondade värvi abil, kui palju on muinsuskaitse all olevaid kirikuid maakondades. Selleks tuleb siduda tabel, kus on kirjas kiriku arv maakonnas, maakondade kihiga ning värvida maakonnad vastavalt kirikute arvule.

Ava Exceli vms programmiga fail *kirikud_maakonniti.csv*. Nagu näha on tegu väga lihtsa andmestikuga, kus ainult kaks tulpa ja 16 rida. **Ava sama fail QGISis: **
...
- Maakondade kihil topeltklikk → vasakul servas *Joins* → all servas roheline plussmärk.
    ...
    - *OK*
    - Vaata uuesti maakondade kihi atribuuttabelit (kiirklahv F6), et näha, kas igale maakonnale said lisatud kirikute arvud.
...
- Nüüd on peaaknas näha maanteed koos reljeefiga. Kui WMS kihid ei paista välja, siis muuda kihtide järjekorda või suumi rohkem sisse või välja.
- Võid proovida lisada ka täiendavaid kihte.
...
- Üleval servas *Web → QuickMapServices → OSM → OSM standard*. Nüüd on aluskaardiks OpenStreetMap.  

Proovi ka teisi kaarte. Nt Google. 

## Järgmisel korral

Kaartide kujundamine ja eksportimine, sümbolite visualiseerimine, andmete loomine ja digitaliseerimine.
