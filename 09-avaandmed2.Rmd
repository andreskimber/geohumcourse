...
Ühendame [Maa-ameti maakonnapiiride andmed](https://geoportaal.maaamet.ee/docs/haldus_asustus/maakond_shp.zip?t=20201001005508) vallakohtute andmetega.  
...
Kahe andmestiku ühendamiseks on nüüd kaks viisi.

**1. Esiteks võime ühendada csv-tabeli shapefile'iga**. Sellisel juhul säilib shapefile'i struktuur, milles on 15 rida (iga maakonna kohta üks) ning csv-failist lisatakse igale reale ainult mingid keskmised näitajad. Võime siin näiteks ühendada shapefile'iga iga maakonna keskmise protokolli pikkuse tähemärkides.
Selleks:  

- leiame keskmise tähemärkide arvu


**2. Teiseks võime ühendada shapefile'i ruumiinfo csv-tabeliga**. Sellisel juhul säilib csv-faili struktuur, milles on üle 25 000 rea (iga protokolli kohta üks) ning iga rida saab juurde koordinaadid. Võime siin näiteks ühendada csv-failiga iga maakonna keskpunkti (*tsentroidi*) koordinaadid. 
...
Teeme heat mapi (mida rohkem punkte on üksteise all, seda intensiivsem on ringi toon).

Otsime välja näiteks teemad ja teeme heat mapi.

Proovi sama kihelkondade andmetega. NB! Kihelkondade andmestikus on mõned polügoonid, mille ruumikuju on vigane ja millel seetõttu ei saa näiteks keskpunkte leida. Selleks, et see segama ei hakkaks, määra praegu 
Settings -> Options -> Processing -> General menu -> Invalid features filtering (= Skip (ignore) invalid geometries). 
