...
Lae [Moodle'ist alla tänase praktikumi andmed](https://moodle.ut.ee/course/view.php?id=10284) ning ava kõik need kihid QGISis. Võid jätkata eelmise praktikumi QGIS projekti. Kihelkondade kiht on kujutab 1917. aasta seisu. Kihis võib olla mõningaid ebatäpsusi. Mälestiste kihis on kõik muinsuskaitse all olevad mälestised. Aladena tähistatud mälestistest on siin lihtsama käsitlemise huvides tehtud samuti punktid. Pärast avamist vaadake ka andmete atribuuttabeleid, et näha millist infot on nendega kaasas. Vorbuse uurimisala kasutame harjutuse käigus ja praegu võib selle välja lülitada.
...
Formaadiks *Geopackage* ja vali sirvimise ikoonile klikkides kursuse kaustas andmete kaust. Anna kihile mõistlik nimi. Näiteks *kihelkonnad_saaremaa* või *saaremaa_kihelkonnad*. Ava see kiht QGISis ja vaata, kas on olemas ikka ainult Saaremaa kihelkonnad.

Vali kaardil Liivimaa kubermangu kihelkonnad. *Select Features By Value* ja kubermangu tulpa kirjuta "Liivimaa" *→ Select Features → Close*. Vali samamoodi kõik arheoloogia mälestised ja salvesta need eraldi kihina. Valiku tegemiseks vaata kihi atribuuttabelit. Neid valikuid salvestama ei pea. Valiku deselekteerimiseks kliki valimise tööriistade kõrval ikoonil *Deselect features from all layers*. Võid proovida olemasolevatest kihtides mõne atribuuttabeli väärtuse põhjal ka teisi valikuid teha. Järgmistel kordadel vaatame ka keerulisemaid valikuid *Select Features By Expression* abil. 
...
- Ajalooliste kaartide kohta leiab rohkem infot ja leppemärgide seletused [Maa-ameti ajalooliste kaartide lehele](https://geoportaal.maaamet.ee/est/Kaardirakendused/Ajaloolised-kaardid/Ajalooliste-kaartide-rakenduse-kirjeldus-p157.html) 
...
Kui kaardiaknas on loodud sobiv lahendus ehk kõik talud ja teed on märgitud, siis ekspordi see kaardina. Kui on tõesti väga kiire, siis võiks hetkeseisust lihtsalt Print Screen teha ning Paint'is vajalik osa välja lõigata. Nii tehes oleks kaart aga seotud praeguse ekraani resolutsiooniga ning meil ei oleks võimalik lisada kaardi kasutamiseks olulisi elemente.
