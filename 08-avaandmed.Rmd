...
Vaata TÜ [DataDOI](https://datadoi.ee/) repositooriumis mõnd jagatud andmestikku/andmekogu ning mõtle, kas ja kui hästi on selle puhul täidetud *FAIR*-põhimõtted.  
...
```{r statamet, echo=F, fig.show='hold', fig.align='center', out.width= "30%", fig.cap = "Mingi murde kõnelejate kuvamine Statistikaameti teemakaardil (1)"}
knitr::include_graphics(here::here("imgs", "statamet.PNG"))
```

```{r statamet2, echo=F, fig.show='hold', fig.align='center', out.width= "80%", fig.cap = "Mingi murde kõnelejate kuvamine Statistikaameti teemakaardil (2)"}
...
+ Nüüd värvi polügoonid äsja loodud tunnuse põhjal: paremklikk kihil -> *Properties* -> *Symbology* -> *Graduated* -> *Value* (= arvulise väärtuse tulp) -> *Color ramp* -> *All Color Ramps* -> *OrRd* -> *Classes* (= 4) -> eemalda linnuke *Link class boundaries* eest -> muuda *Values* väärtusi nii, et need vastaksid statistikaameti kaardile -> *Classify* -> *OK*.  
...
- [INSPIRE](https://inspire-geoportal.ec.europa.eu/) geoportaal koondab Euroopa Liidu liikmesriikide  ja EFTA riikide ruumiandmeid (sh Eesti andmeid Maa-ameti kaudu).  
...
Esmaspäeval tegeleme georefereerimisega ehk seome skaneeritud kaarte geograafiliste koordinaatidega.