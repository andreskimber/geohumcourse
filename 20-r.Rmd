...
Võimalusi ja ideid graafikute ja kaartide koostamiseks R-is leiab [R Graph Gallery'ist](https://www.r-graph-gallery.com/).  
...
malestised = st_read("./andmed/malestised.gpkg")
...
Soovime näiteks teha kaarti Saaremaa asustusüksustest, mille esmamainimine on olnud hiljemalt aastal 1558.
...
Teeme esmamainimise aastatest ka ülevaatliku graafiku

```{r}
graafik = ggplot2::ggplot(saare_asustus) + 
    geom_histogram(
        mapping = aes(x = esmamainimine),
        binwidth = 50,
        ) +
    theme_clean()
graafik
...
Filtreerime välja õige vanusega külad. 

```{r}

summary(saare_asustus$esmamainimine)
...
ggplot() +
    geom_sf(
        saaremaa, mapping = ggplot2::aes(), 
        show.legend = T
        ) +
    geom_sf(
        data = saare_asustus_vana,
...
Teeme mitu kaarti korraga. Selleks loome tulba, kus kirjas esmamainimise ajastu. 

```{r}
saare_asustus_periood = saare_asustus %>% 
    mutate(
        esmamainimise_periood = case_when(
            esmamainimine < 1558 ~ "Keskaeg", 
            esmamainimine < 1721 ~ "Vara-uusaeg",
            esmamainimine < 1918 ~ "Uusaeg",
            esmamainimine < 2021 ~ "Uusim aeg",
        )
    )
...
ggplot() +
    ...
    annotation_scale(location = "br") + # lisame mõõtkava. "br" tähendam bottom right
    labs(
        title = "Saaremaa külade esmamainimine ajaloo perioodide kaupa",
        caption = "Andmed: Maa-amet 2020, EKI Kohanimeraamat"
        ) +
    # peidame taustal oleva koordinaatteljestiku
    theme(
    ...
    facet_wrap(~ esmamainimise_periood)
```
...
Tee kaart, kus on kujutatud Tartu maakonna asustusüksusi, mille esmamainimine jääb Põhjasõja (1700–1721) eelsesse aega. Kirjuta kood all olevasse kooditükki. Saad kombineerida täpselt sama koodi, mida varem kasutasime.
...
tartu_asustus_vana = tartu_asustus %>% 
    filter(
        esmamainimine < 1700
    )
...
ggplot() +
    geom_sf(
        tartumaa, mapping = ggplot2::aes(), 
        show.legend = T
        ) +
    geom_sf(
        data = tartu_asustus_vana,
        mapping = ggplot2::aes(color = esmamainimine), 
        size = 3
        ) +
    coord_sf(crs = 3301) +
    annotation_scale(location = "br") + # lisame mõõtkava. "br" tähendam bottom right
    labs(
        title = "Tartu maakonna asustus aastani 1700",
        caption = "Andmed: Maa-amet 2020, EKI Kohanimeraamat"
        ) +
    # peidame taustal oleva koordinaatteljestiku
    theme(
