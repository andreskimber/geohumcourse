...
Maakera on kolmemõõtmeline ja füüsiliselt ebakorrapärane. <!-- Tegelikult annab kõige parema ettekujutuse maakera kujust geoid ehk maa gravitatsioonivälja pind. Kuna maakera mass ei ole kõigis punktides ühesugune ja gravitatsiooni suund muutub, on geoidi kuju korrapäratu. --> Kaardil on üldjuhul aga ainult kaks mõõdet. Kuidas siis kolmemõõtmelist ning ebakorrapärast kujundit kujutada kahemõõtmelise ja korrapärase meediumi kaudu ning kuidas paigutada reaalse maailma objekte ja alasid nii, et saaksime kaardi põhjal öelda, kus need maakeral asuvad? <!-- tuleb mingeid alasid venitada ja mingeid kokku suruda --> 
...
```{r, echo = F, fig.align="centre", fig.show="hold", fig.cap="Ellipsi lapikus"}
library(ggplot2)
library(ggforce)

ggplot() +
    geom_ellipse(aes(x0 = 0, y0 = 0, a = 4, b = 4, angle = 0), color = "tomato1") +
    geom_ellipse(aes(x0 = 0, y0 = 0, a = 4, b = 3, angle = 0), color = "seagreen3") +
    geom_ellipse(aes(x0 = 0, y0 = 0, a = 4, b = 2, angle = 0), color = "steelblue3") +
    geom_ellipse(aes(x0 = 0, y0 = 0, a = 4, b = 1, angle = 0), color = "orange3") +
    geom_segment(aes(x = -4, xend = 4, y = 0, yend = 0), linetype = "dashed", col = "grey50") +
    geom_segment(aes(x = 0, xend = 0, y = -4, yend = 4), linetype = "dashed", col = "grey50") +
    geom_segment(aes(x = 0, xend = 4, y = 0, yend = 0), linetype = "solid", col = "grey25") +
    geom_segment(aes(x = 0, xend = 0, y = 0, yend = 1), linetype = "solid", col = "grey25") +
    geom_text(aes(x = 0, y = 4, label = "0"), color = "tomato1", hjust = -0.1, vjust = -0.1) +
    geom_text(aes(x = 0, y = 3, label = "0.25"), color = "seagreen3", hjust = -0.1, vjust = -0.1) +
    geom_text(aes(x = 0, y = 2, label = "0.5"), color = "steelblue3", hjust = -0.1, vjust = -0.1) +
    geom_text(aes(x = 0, y = 1, label = "0.75"), color = "orange3", hjust = -0.1, vjust = -0.1) +
    geom_text(aes(x = 2, y = 0, label = "a"), color = "grey25", vjust = -0.2) +
    geom_text(aes(x = 0, y = 0.5, label = "b"), color = "grey25", hjust = -0.2) +
    coord_fixed() +
    labs(x = "", y = "") +
    theme_bw()
```
...
Selleks aga, et nüüd geograafilises ruumis paiknevaid objekte kujutada tasapinnalisel mudelil ehk kaardil, peame valima kaardi **projektsiooni**, mis määrab viisi, kuidas ruumilised koordinaadid konverteeritakse ümber **tasapinnalisteks koordinaatideks**. Sisuliselt määrab projektsioon ära selle, milliseid alasid maakeral me oleme nõus oma kaardil rohkem moonutama ja milliseid vähem ning mispidi me mingeid alasid kaardil venitame või kokku surume. 
...
Kasutusel on aga ka tasapinnalised **ristkoordinaadid**, mida väljendatakse X' ja Y' abil. Näiteks Eestis üldiselt kasutatavas [koordinaatide referentssüsteemis](http://epsg.io/3301) esitatakse koordinaate just ristkoordinaatide kaudu.
